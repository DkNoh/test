...
function loadPending(type) {
    fetch(`/campaign/${type}/pending`)
        ...
        .then(res => {
            ...
            if (!list.length) {
                tbody.innerHTML = '<tr><td colspan="6" class="text-center text-muted py-4">승인 대기 건이 없습니다.</td></tr>';
                return;
            }
...
async function process(campaignId, action) {
    const msg = action === 'APPROVED' ? '승인' : '반려';
    CommonUtils.confirm(`해당 캠페인을 ${msg}하시겠습니까?`, async () => {
        try {
            await axios.post('/campaign/approve', { campaignId, action });
            CommonUtils.toast(`${msg} 처리되었습니다.`, 'success');
            loadPending('sms');
        } catch (e) {}
    });
}
