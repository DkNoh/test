document.addEventListener('DOMContentLoaded', () => loadPending('alimtalk'));
...
async function process(campaignId, action) {
    CommonUtils.confirm(`해당 캠페인을 ${action==='APPROVED'?'승인':'반려'}하시겠습니까?`, async () => {
        try { await axios.post('/campaign/approve', {campaignId, action}); CommonUtils.toast('처리되었습니다.', 'success'); loadPending('alimtalk'); } catch(e) {}
    });
}
